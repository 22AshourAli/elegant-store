<x-guest-layout>
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        ...
        <div x-data="{
            ...
            get error() { return this.touched && this.value.length === 0 ? '{{ __('global.field_required') }}' : (this.touched && this.value.length > 0 && !this.valid ? '{{ __('global.name_min_length') }}' : ''); }
        ...
        <div x-data="{
            ...
            get error() { return this.touched && this.value.length === 0 ? '{{ __('global.field_required') }}' : (this.touched && this.value.length > 0 && !this.valid ? '{{ __('global.email_invalid') }}' : ''); }
        }" @input="touched = true" class="group">
        ...
        <div x-data="{
            ...
            get formatted() {
                ...
                if (d.length <= 5) return '+20 ' + d.slice(0, 3) + ' ' + d.slice(3);
                return '+20 ' + d.slice(0, 3) + ' ' + d.slice(3, 5) + ' ' + d.slice(5, 11);
            },
            get valid() {
                const d = this.digits;
            ...
            <div class="relative mt-1.5">
                <div class="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none z-10">
                    <span class="text-xs font-black text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded-md" :class="valid && value.length > 0 ? 'text-emerald-500 bg-emerald-50 dark:bg-emerald-900/30' : ''">+20</span>
                </div>
                <input id="phone" type="tel" name="phone" x-model="value" @input="formatInput(); touched = true" @blur="touched = true"
                       autocomplete="tel"
                       placeholder="100 000 0000"
                       inputmode="numeric"
                       class="block w-full ps-16 pe-10 rounded-xl border-2 bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm text-sm transition-all duration-200 py-2.5 outline-none tracking-wider font-mono"
                       :class="error ? 'border-red-300 dark:border-red-700 focus:border-red-500 focus:ring-red-500/20' : (valid && value.length > 0 ? 'border-emerald-300 dark:border-emerald-700 focus:border-emerald-500 focus:ring-emerald-500/20' : 'border-slate-200 dark:border-slate-700 focus:border-indigo-500 focus:ring-indigo-500/20')"
                       :style="error ? 'box-shadow: 0 0 0 3px rgba(239,68,68,0.1)' : (valid && value.length > 0 ? 'box-shadow: 0 0 0 3px rgba(16,185,129,0.1)' : '')">
                <div class="absolute inset-y-0 end-0 flex items-center pe-3">
                    <template x-if="value.length > 0 && !error">
            ...
            {{-- Format preview --}}
            <div x-show="value.length > 0" x-transition class="mt-1.5 text-[10px] font-mono font-bold text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800/50 rounded-lg px-2.5 py-1 inline-block">
                <span class="text-slate-300 dark:text-slate-600">+20 </span>
                <span x-text="formatted.replace('+20 ', '')" class="text-slate-500 dark:text-slate-400"></span>
            </div>
            <template x-if="error">
                <p class="mt-1.5 text-xs font-semibold text-red-500 flex items-center gap-1" x-text="error"></p>
            </template>
            {{-- Hint when empty --}}
            <p x-show="value.length === 0" class="mt-1.5 text-[10px] font-medium text-slate-400 dark:text-slate-500 flex items-center gap-1">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div x-data="{
                ...
                <div class="relative mt-1.5">
                    <input id="password" name="password" x-model="value" @blur="touched = true" x-bind:type="show ? 'text' : 'password'" required autocomplete="new-password"
                           placeholder="&#8226;&#8226;&#8226;&#8226;&#8226;&#8226;&#8226;&#8226;"
                           class="block w-full pe-10 rounded-xl border-2 bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm text-sm transition-all duration-200 py-2.5 outline-none ps-3"
                           :class="error ? 'border-red-300 dark:border-red-700 focus:border-red-500 focus:ring-red-500/20' : (valid && value.length > 0 ? 'border-emerald-300 dark:border-emerald-700 focus:border-emerald-500 focus:ring-emerald-500/20' : 'border-slate-200 dark:border-slate-700 focus:border-indigo-500 focus:ring-indigo-500/20')"
                           :style="error ? 'box-shadow: 0 0 0 3px rgba(239,68,68,0.1)' : (valid && value.length > 0 ? 'box-shadow: 0 0 0 3px rgba(16,185,129,0.1)' : '')">
            ...
            <div x-data="{
                ...
                get error() { return this.touched && this.value.length === 0 ? '{{ __('global.field_required') }}' : (this.touched && this.value.length > 0 && !this.valid ? '{{ __('global.password_mismatch') }}' : ''); }
            }" @input="touched = true" class="group">
        ...
        <button type="submit"
            ...
            <span class="relative z-10 flex items-center justify-center gap-2">
                <svg class="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
                {{ __('global.sign_up') }}
            </span>
        </button>
