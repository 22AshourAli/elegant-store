        ...
        <div class="mt-2 flex items-start justify-between gap-2">
            <div class="min-w-0">
                ...
                @if($product->isOnSale)
                    <p class="text-[10px] text-emerald-600/80 dark:text-emerald-400/80 font-bold mt-0.5 flex items-center gap-0.5" dir="auto">
                        ...
                        <span>{{ __('global.save_label') }} {{ $fmtNum($product->base_price - $product->current_price) }} {{ __('global.currency') }}</span>
                    </p>
                @endif
            </div>
            ...
            <div class="flex-shrink-0 text-end">
                <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 leading-relaxed whitespace-nowrap">
                    {{ __('global.available_label') }}
                    @if($hasColorVar){{ $fmtNum($allColors->count()) }} {{ __('global.colors_count') }}@endif
                    @if($hasColorVar && $hasSizeVar) {{ __('global.and_separator') }} @endif
                    @if($hasSizeVar){{ $fmtNum($allSizes->count()) }} {{ __('global.sizes_count') }}@endif
                </span>
            </div>
            @endif
        </div>
