...
@section('page-title', __('global.admin_bulk_results'))
@section('content')
<div class="bg-white dark:bg-gray-800 rounded shadow p-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <div class="bg-green-50 dark:bg-green-950/30 rounded-lg p-3 text-center">
            ...
            <p class="text-xs text-green-600 dark:text-green-400 mt-1">{{ __('global.admin_email_sent') }}</p>
        </div>
        @endif
        ...
        <div class="bg-red-50 dark:bg-red-950/30 rounded-lg p-3 text-center">
            ...
            <p class="text-xs text-red-600 dark:text-red-400 mt-1">{{ __('global.admin_email_failed') }}</p>
        </div>
        @endif
        <div class="bg-amber-50 dark:bg-amber-950/30 rounded-lg p-3 text-center">
            ...
            <p class="text-xs text-amber-600 dark:text-amber-400 mt-1">{{ __('global.admin_whatsapp_links') }}</p>
        </div>
    </div>

    @if(count($results) > 0)
    <div class="flex items-center justify-between mb-4">
        <h3 class="font-bold text-lg">{{ __('global.admin_whatsapp_links') }} ({{ count($results) }})</h3>
        <button onclick="copyAllLinks()"
            ...
            {{ __('global.admin_copy_all_links') }}
        </button>
    </div>

    <div class="text-sm text-gray-500 mb-4 p-4 bg-amber-50 dark:bg-amber-950/20 rounded">
        <strong>{{ __('global.admin_note_colon') }}</strong> {{ __('global.admin_wa_links_desc_1') }} <strong>{{ __('global.admin_new_tab') }}</strong>.
        {{ __('global.admin_wa_links_desc_2') }}
    </div>
        ...
        <table class="w-full text-sm text-right">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    ...
                    <th class="p-3">{{ __('global.admin_phone_number') }}</th>
                    <th class="p-3">{{ __('global.admin_action') }}</th>
                </tr>
            </thead>
            <tbody>
...
function copyLink(url, btn) {
    navigator.clipboard.writeText(url).then(function() {
        ...
        btn.innerHTML = '{{ __('global.admin_copied_check') }}';
...
function copyAllLinks() {
    ...
    navigator.clipboard.writeText(links).then(function() {
        alert('{{ __('global.admin_all_links_copied') }}');
    });
}
</script>
@endpush
