...
@section('page-title', __('global.admin_whatsapp_marketing'))
@section('content')
    ...
    <table class="w-full text-sm text-right">
        <thead class="bg-gray-50 dark:bg-gray-700">
            <tr>
                ...
                <th class="p-3 hidden md:table-cell">{{ __('global.admin_total_orders') }}</th>
                <th class="p-3 hidden md:table-cell">{{ __('global.admin_total_spent') }}</th>
                <th class="p-3 hidden md:table-cell">{{ __('global.admin_last_message') }}</th>
                <th class="p-3 hidden md:table-cell">{{ __('global.admin_last_sent') }}</th>
                ...
                <tr class="border-t dark:border-gray-700 even:bg-gray-50/50 dark:even:bg-gray-700/20">
                    <td class="p-3">
                            <div class="flex items-center gap-1.5">
                                ...
                                @else
                                    <span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-gray-100 text-gray-500">{{ __('global.admin_new') }}</span>
                                @endif
                            </div>
                        </td>
                    <td class="p-3 hidden md:table-cell">{{ $customer->total_orders }}</td>
                    <td class="p-3 hidden md:table-cell">{{ number_format((int) $customer->total_spent) }} {{ __('global.currency') }}</td>
                    ...
                    <td class="p-3 text-right">
                        <a href="{{ route('admin.whatsapp.show', ['user' => $customer->id]) }}" class="inline-flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white text-xs font-bold rounded-lg hover:bg-green-700 transition">
                            ...
                            {{ __('global.admin_send_message') }}
                        </a>
                    </td>
                </tr>
            @empty
                <tr><td colspan="6" class="p-6 text-center text-gray-400">{{ __('global.admin_no_customers') }}</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
