...
@section('page-title', __('global.edit_expense'))
@section('content')
<div class="max-w-lg mx-auto bg-white dark:bg-gray-800 rounded shadow p-6">
    <form action="{{ route('admin.expenses.update', $expense) }}" method="POST">
        ...
        <div class="space-y-4">
            ...
            <button type="submit" class="w-full bg-indigo-600 text-white font-bold py-2.5 rounded hover:bg-indigo-700 transition text-sm">{{ __('global.update_expense') }}</button>
        </div>
    </form>
</div>
@endsection
