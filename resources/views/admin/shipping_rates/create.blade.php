...
@section('page-title', __('global.admin_add_shipping_rate'))
@section('content')
<div class="bg-white dark:bg-gray-800 rounded shadow p-6 max-w-2xl mx-auto">
    <form action="{{ route('admin.shipping-rates.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="shipping_provider_id" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white px-3 py-2 border">
                ...
                @foreach($providers as $provider)
                    <option value="{{ $provider->id }}" {{ old('shipping_provider_id') == $provider->id ? 'selected' : '' }}>{{ $provider->name }}</option>
                @endforeach
            </select>
            @error('shipping_provider_id') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>

        <div class="mb-4">
            <label class="block text-sm font-medium mb-1">{{ __('global.admin_rate') }} ({{ __('global.currency') }}) <span class="text-red-500">*</span></label>
            <input type="number" step="0.01" min="0" name="rate" required value="{{ old('rate') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white px-3 py-2 border">
        ...
        <div class="mb-4">
            ...
            <input type="number" step="0.01" min="0" name="min_cart_amount" value="{{ old('min_cart_amount') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white px-3 py-2 border">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="estimated_days" value="{{ old('estimated_days') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white px-3 py-2 border">
        </div>
