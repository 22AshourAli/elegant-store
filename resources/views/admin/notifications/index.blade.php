        ...
        <div class="divide-y divide-gray-100 dark:divide-gray-700">
            @foreach($notifications as $notification)
                ...
                <div class="flex items-start gap-4 p-4 transition {{ $isUnread ? 'bg-indigo-50/50 dark:bg-indigo-950/20 border-r-4 border-r-indigo-500' : 'opacity-70' }}">
                    <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center {{ isset($notifData['order_id']) ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' }}">
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        @if(isset($notifData['order_id']))
                            <a href="{{ route('admin.orders.show', $notifData['order_id']) }}" class="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">{{ __('global.admin_view_update') }}</a>
                        @endif
                        @if($isUnread)
