            ...
            @foreach($notifications as $notification)
                ...
                @endphp
                    @php
                        ...
                        if (in_array($notifType, ['exchange', 'exchange_approved', 'exchange_submitted'])) {
                            ...
                            $notifUrl = isset($notifData['return_request_id'])
                                ...
                                : null;
                        } elseif (isset($notifData['review_id'])) {
                            $notifUrl = route('admin.reviews.show', $notifData['review_id']);
                    ...
                    <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center
                        {{ in_array($notifType, ['exchange', 'exchange_approved', 'exchange_submitted']) ? 'bg-green-100 dark:bg-green-900/30 text-green-600' : ($notifType === 'return' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' : (isset($notifData['review_id']) ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600' : (isset($notifData['order_id']) ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600' : 'bg-gray-100 dark:bg-gray-700 text-gray-500'))) }}">
                        @if(in_array($notifType, ['exchange', 'exchange_approved', 'exchange_submitted']))
