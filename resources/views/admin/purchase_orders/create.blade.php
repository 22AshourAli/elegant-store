...
@section('page-title', __('global.admin_add_po'))
@section('content')
    ...
    <form action="{{ route('admin.purchase-orders.store') }}" method="POST" x-data="poForm()">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div>
                ...
                <input type="date" name="expected_at" value="{{ old('expected_at') }}" class="w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200">
                @error('expected_at') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>
        </div>
                ...
                <div class="flex-1">
                    <select :name="'items[' + index + '][variant_id]'" x-model="item.variant_id" required class="w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-200">
                        ...
                        @foreach($variants as $variant)
                            <option value="{{ $variant->id }}" data-price="{{ $variant->cost_price ?? 0 }}">{{ $variant->sku }} - {{ $variant->product->name ?? '' }} ({{ $variant->color }} / {{ $variant->size }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="w-24">
        ...
        <div class="flex gap-2 mt-6">
            ...
            <a href="{{ route('admin.purchase-orders.index') }}" class="bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 px-4 py-2 rounded hover:bg-gray-300 dark:hover:bg-gray-500">{{ __('global.admin_cancel') }}</a>
        </div>
    </form>
</div>
@endsection
...
function poForm() {
    return {
        ...
        addItem() {
            this.items.push({ variant_id: '', quantity: 1, unit_cost: 0 });
        },
        removeItem(index) {
