...
@section('page-title', 'تسوية المدفوعات')
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-700/60 p-6 text-center">
        <p class="text-sm text-slate-500 dark:text-slate-400 font-bold mb-1">إجمالي المدفوعات</p>
        <p class="text-3xl font-black text-brand-primary">{{ number_format($data['gross_revenue'] ?? 0) }} {{ __('global.currency') }}</p>
    </div>
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-700/60 p-6 text-center">
        <p class="text-sm text-slate-500 dark:text-slate-400 font-bold mb-1">إجمالي الرسوم</p>
        <p class="text-3xl font-black text-rose-600">{{ number_format($data['total_gateway_fees'] ?? 0) }} {{ __('global.currency') }}</p>
        <p class="text-[10px] text-slate-400 mt-1">({{ $data['fee_percent'] ?? 0 }}% من الإيرادات)</p>
    </div>
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-700/60 p-6 text-center">
        <p class="text-sm text-slate-500 dark:text-slate-400 font-bold mb-1">صافي المبلغ</p>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-700/60 p-6">
    <h3 class="font-extrabold text-lg mb-4 text-slate-900 dark:text-white">حسب بوابة الدفع</h3>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-slate-50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider">
                    <th class="text-right px-4 py-3">البوابة</th>
                    <th class="text-center px-4 py-3">المعاملات</th>
                    <th class="text-center px-4 py-3">الإجمالي</th>
                    <th class="text-center px-4 py-3">الرسوم</th>
                    <th class="text-center px-4 py-3">الصافي</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                ...
                @empty
                <tr><td colspan="5" class="text-center py-6 text-slate-400">لا توجد بيانات</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
