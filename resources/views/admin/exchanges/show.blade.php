...
@section('page-title', 'طلب استبدال #' . $exchange->id)
@section('content')
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white dark:bg-gray-800 rounded shadow p-6">
            ...
            <div class="space-y-3 text-sm">
                <div><span class="font-semibold">العميل:</span> {{ $exchange->user->name }} ({{ $exchange->user->email }})</div>
                <div><span class="font-semibold">الطلب رقم:</span> #{{ $exchange->order_id }}</div>
                <div><span class="font-semibold">الحالة:</span> {{ $exchange->status }}</div>
                <div><span class="font-semibold">تاريخ الطلب:</span> {{ $exchange->created_at->format('Y-m-d H:i') }}</div>
