...
async function openCustomerModal(id) {
    ...
    modal.innerHTML = `
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onclick="document.getElementById('customer-modal')?.remove()">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto p-6" onclick="event.stopPropagation()">
                <div class="flex items-start gap-4 mb-6">
                    <img src="${data.avatar}" class="w-14 h-14 rounded-full border-2 border-slate-200 dark:border-slate-600">
                    ...
                    <button onclick="document.getElementById('customer-modal')?.remove()" class="text-slate-400 hover:text-slate-600 transition"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                </div>
