        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'"
            ...
            <div class="p-4 flex-1 overflow-y-auto">
                <nav class="space-y-1.5">
                    ...
                    <a href="{{ route('admin.orders.index') }}" x-data="{ orderCount: 0 }" @order-count-updated.window="orderCount = $event.detail.count" class="relative flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition duration-200 {{ request()->routeIs('admin.orders.*') ? 'bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 font-semibold' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 hover:text-gray-900 dark:hover:text-white' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
                        </svg>
                        {{ __('global.admin_orders') }}
                        <span x-show="orderCount > 0" class="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full animate-ping" style="display:none"></span>
                    </a>
                    ...
                    <a href="{{ route('admin.reviews.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition duration-200 {{ request()->routeIs('admin.reviews.*') ? 'bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 font-semibold' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 hover:text-gray-900 dark:hover:text-white' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                        </svg>
                        {{ __('global.admin_reviews') }}
                    </a>
                    ...
                    <a href="{{ route('admin.stock-counts.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition duration-200 {{ request()->routeIs('admin.stock-counts.*') ? 'bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 font-semibold' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 hover:text-gray-900 dark:hover:text-white' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
                        </svg>
                        {{ __('global.admin_stock_counts') }}
                    </a>
                    ...
                    <div class="pt-3 mt-3 border-t border-slate-200 dark:border-slate-700">
                        ...
                        <a href="{{ route('admin.backups.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition duration-200 {{ request()->routeIs('admin.backups.*') ? 'bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 font-semibold' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 hover:text-gray-900 dark:hover:text-white' }}">
                            ...
                            {{ __('global.admin_backups') }}
                        </a>
                        @if(auth()->user()->isSuperAdmin())
                        <a href="{{ route('admin.activity-logs.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition duration-200 {{ request()->routeIs('admin.activity-logs.*') ? 'bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 font-semibold' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 hover:text-gray-900 dark:hover:text-white' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                            </svg>
                            {{ __('global.admin_activity_logs') }}
                        </a>
                        @endif
                    </div>
                    <div class="pt-3 mt-3 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <div x-data="notifications()" x-init="init()">
                        <div class="relative cursor-pointer">
                            <button @click="togglePanel" class="relative p-1.5 sm:p-2 cursor-pointer rounded-xl bg-gray-50 dark:bg-gray-800 text-gray-500 hover:scale-105 transition duration-150">
                                ...
                                <span x-show="unread > 0" class="absolute -top-1 -right-1 w-[18px] h-[18px] bg-red-400 rounded-full animate-ping" style="display:none"></span>
                                    ...
                                    <template x-for="n in items" :key="n.id">
                                        <a :href="n.url || '#'" @click="n.read_at ? null : markRead(n.id)" class="block px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition border-b border-gray-50 dark:border-gray-700/50" :class="{'bg-indigo-50/50 dark:bg-indigo-950/20': !n.read_at}">
                                            <div class="flex items-start gap-3">
                                                <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center" :class="n.type === 'exchange' ? 'bg-green-100 dark:bg-green-900/30 text-green-600' : n.type === 'return' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' : n.type === 'review' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600' : n.type === 'order' ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600' : 'bg-gray-100 dark:bg-gray-700 text-gray-500'">
                            ...
                            <a :href="toastItem.url || '#'" @click="dismissToast(); toastItem.read_at ? null : markRead(toastItem.id)"
                               class="flex items-start gap-3 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/60 dark:border-gray-700/60 p-4 hover:shadow-xl transition-shadow">
                                <div class="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center shadow-sm"
                                     :class="toastItem.type === 'exchange' ? 'bg-emerald-100 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400' : toastItem.type === 'return' ? 'bg-amber-100 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400' : toastItem.type === 'review' ? 'bg-yellow-100 dark:bg-yellow-950/30 text-yellow-600 dark:text-yellow-400' : toastItem.type === 'order' ? 'bg-indigo-100 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'">
                                    ...
                                    <template x-if="toastItem.type === 'return'">
                                     ...
                                     <template x-if="toastItem.type === 'review'">
            ...
            Alpine.data('notifications', () => ({
                ...
                async fetchUnread() {
                    try {
                        ...
                        window.dispatchEvent(new CustomEvent('order-count-updated', { detail: { count: data.order_count || 0 } }));
                        this._updateTitle();
                        return true;
