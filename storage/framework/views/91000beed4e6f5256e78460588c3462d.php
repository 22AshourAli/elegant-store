<?php $inWish = in_array($product->id, $wishlistIds ?? []); $inCart = in_array($product->id, $cartProductIds ?? []); ?>
<article class="group product-card relative overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:glow-indigo"
     x-data="{ inWishlist: <?php echo e($inWish ? 'true' : 'false'); ?>, inCart: <?php echo e($inCart ? 'true' : 'false'); ?>, removed: false }"
     x-show="!removed"
    ...
    <a href="<?php echo e(route('shop.product', $product->slug)); ?>" class="block relative overflow-hidden aspect-[4/5] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2 rounded-t-2xl" aria-label="<?php echo e($product->name); ?> details">
        <?php
        ...
        ?>
        <img src="<?php echo e($image); ?>" alt="<?php echo e($product->name); ?> - <?php echo e($descText); ?> - Elegant Store" loading="lazy" class="w-full h-full object-cover transition-transform duration-700 ease-out transform-gpu group-hover:scale-110 group-hover:brightness-105">
    ...
    <div class="p-4 text-start bg-white dark:bg-surface-dark">
        <h3 class="text-xs sm:text-sm font-extrabold tracking-tight text-slate-800 dark:text-slate-100 mb-1.5 line-clamp-2 min-h-[2.25rem] sm:min-h-[2.75rem]">
            <a href="<?php echo e(route('shop.product', $product->slug)); ?>" class="hover:text-brand-primary dark:hover:text-accent text-slate-900 dark:text-white transition-colors duration-300 inline-block focus-visible:outline-none focus-visible:underline">
        ...
        <div class="flex items-baseline gap-1.5 sm:gap-2 mt-2">
            <?php if($product->isOnSale): ?>
                <span class="text-sm sm:text-base font-black text-brand-primary dark:text-accent tracking-tight"><?php echo e((int) round($product->current_price)); ?> <?php echo e(__('global.currency')); ?></span>
                <span class="text-[10px] sm:text-xs text-slate-400 dark:text-slate-500 line-through font-bold"><?php echo e((int) round($product->base_price)); ?> <?php echo e(__('global.currency')); ?></span>
            <?php else: ?>
                <span class="text-sm sm:text-base font-black text-brand-primary dark:text-accent tracking-tight"><?php echo e((int) round($product->current_price)); ?> <?php echo e(__('global.currency')); ?></span>
            <?php endif; ?>
        </div>
        ...
        <div class="flex items-center gap-2 mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/60">
            ...
            <button type="button" @click="
                fetch('<?php echo e(route('cart.add', $firstVariant)); ?>', { method: 'POST', credentials: 'same-origin', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>', 'Accept': 'application/json' } })
                    .then(r => r.json())
                    ...
                    .catch(() => window.dispatchEvent(new CustomEvent('toast', { detail: { message: '<?php echo e(__('global.error_adding_to_cart')); ?>', type: 'error' } })));
            ...
            :title="inCart ? '<?php echo e(__('global.added_to_cart')); ?>' : '<?php echo e(__('global.add_to_cart')); ?>'"
            :aria-label="inCart ? '<?php echo e(__('global.added_to_cart')); ?>' : '<?php echo e(__('global.add_to_cart')); ?>'">
            ...
            <button type="button" @click="
                fetch('<?php echo e(route('wishlist.toggle', $product->id)); ?>', { method: 'POST', credentials: 'same-origin', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>', 'Accept': 'application/json' } })
                    .then(r => r.json())
                    .then(d => { inWishlist = d.added; if(!d.added && <?php echo e(($hideOnRemove ?? false) ? 'true' : 'false'); ?>) removed = true; window.dispatchEvent(new CustomEvent('toast', { detail: { message: d.message, type: d.added ? 'success' : 'info' } })); window.dispatchEvent(new CustomEvent('wishlist-updated', { detail: { count: d.count } })); })
                    .catch(() => window.dispatchEvent(new CustomEvent('toast', { detail: { message: '<?php echo e(__('global.login_required')); ?>', type: 'error' } })));
            " class="p-2.5 rounded-xl border transition-all duration-300 hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary cursor-pointer"
            ...
            :title="inWishlist ? '<?php echo e(__('global.remove_from_wishlist')); ?>' : '<?php echo e(__('global.add_to_wishlist')); ?>'"
            :aria-label="inWishlist ? '<?php echo e(__('global.remove_from_wishlist')); ?>' : '<?php echo e(__('global.add_to_wishlist')); ?>'">
            ...
            <?php else: ?>
            <a href="<?php echo e(route('login', ['redirect' => url()->current()])); ?>" class="p-2.5 rounded-xl border border-slate-200/60 dark:border-slate-800/80 text-slate-400 hover:text-red-500 hover:border-red-500/40 dark:hover:border-red-500/40 transition-all duration-300 hover:scale-105 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary"
                title="<?php echo e(__('global.add_to_wishlist')); ?>"
                aria-label="<?php echo e(__('global.add_to_wishlist')); ?>">
...
<?php /**PATH C:\Users\GH\Desktop\Projects\elegant-store\resources\views/shop/partials/product-card.blade.php ENDPATH**/ ?>