...
<section x-data="{
    ...
    interval: null,
    paused: false,
    slides: <?php echo e(Js::from($slides)); ?>,
    touchStartX: 0,
    touchEndX: 0,
    ...
    handleTouchEnd(e) {
        ...
        if (this.touchStartX - this.touchEndX > threshold) {
            if ('<?php echo e(app()->getLocale()); ?>' === 'ar') this.prev(); else this.next();
        } else if (this.touchEndX - this.touchStartX > threshold) {
            if ('<?php echo e(app()->getLocale()); ?>' === 'ar') this.next(); else this.prev();
        }
    }
}" x-init="start()"
...
<section class="anim-fade-up container pt-4 sm:pt-6">
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        <div class="glass-premium dark:bg-slate-900/60 rounded-2xl p-4 sm:p-6 text-center border border-slate-200 dark:border-slate-800/60 shadow-sm hover:shadow-md hover:border-luxury-gold/30 dark:hover:border-luxury-gold/45 hover:scale-[1.03] transition-all duration-300">
            ...
            <div class="text-xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight"><?php echo e($products->total() ?? 0); ?>+</div>
            <div class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-1 font-bold uppercase tracking-wider"><?php echo e(__('global.product')); ?></div>
        </div>
        <div class="glass-premium dark:bg-slate-900/60 rounded-2xl p-4 sm:p-6 text-center border border-slate-200 dark:border-slate-800/60 shadow-sm hover:shadow-md hover:border-luxury-gold/30 dark:hover:border-luxury-gold/45 hover:scale-[1.03] transition-all duration-300">
            ...
            <div class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-1 font-bold uppercase tracking-wider"><?php echo e(__('global.quality_guaranteed')); ?></div>
        </div>
        <div class="glass-premium dark:bg-slate-900/60 rounded-2xl p-4 sm:p-6 text-center border border-slate-200 dark:border-slate-800/60 shadow-sm hover:shadow-md hover:border-luxury-gold/30 dark:hover:border-luxury-gold/45 hover:scale-[1.03] transition-all duration-300">
            ...
            <div class="text-xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight"><?php echo e(__('global.fast_delivery')); ?></div>
            <div class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-1 font-bold uppercase tracking-wider"><?php echo e(__('global.delivery_desc')); ?></div>
        </div>
        <div class="glass-premium dark:bg-slate-900/60 rounded-2xl p-4 sm:p-6 text-center border border-slate-200 dark:border-slate-800/60 shadow-sm hover:shadow-md hover:border-luxury-gold/30 dark:hover:border-luxury-gold/45 hover:scale-[1.03] transition-all duration-300">
            ...
            <div class="text-xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight"><?php echo e(__('global.secure_payment')); ?></div>
            <div class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 mt-1 font-bold uppercase tracking-wider"><?php echo e(__('global.payment_desc')); ?></div>
        </div>
    </div>
</section>
...
<section class="anim-fade-up container py-12 sm:py-16 md:py-20">
    <div class="flex items-center justify-between mb-8 sm:mb-10">
        <h2 class="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-white relative inline-block">
            <?php echo e(__('global.shop_by_category')); ?>
                ...
                <a href="<?php echo e(route('shop.category', $cat->slug)); ?>"
                   class="group relative bg-white dark:bg-[#0b0f19] rounded-2xl overflow-hidden border border-slate-200/50 dark:border-slate-900/80 hover:border-luxury-gold/30 dark:hover:border-accent/40 aspect-[4/3] flex items-center justify-center p-5 text-center shadow-sm hover:shadow-[0_20px_40px_-15px_rgba(79,70,229,0.15)] dark:hover:shadow-[0_20px_45px_-10px_rgba(0,0,0,0.5)] transition-all duration-500 hover:-translate-y-2 hover:scale-[1.02] focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:outline-none">
                    ...
                    <div class="relative z-10 flex flex-col items-center">
                        <span class="text-base sm:text-lg font-extrabold text-slate-800 dark:text-slate-100 group-hover:text-brand-primary dark:group-hover:text-accent transition-colors block line-clamp-2"><?php echo e($cat->name); ?></span>
                        <?php if($cat->children->count() > 0): ?>
                            <span class="inline-flex items-center gap-1 text-[10px] font-bold text-brand-primary/80 dark:text-luxury-gold/90 bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200/30 dark:border-luxury-gold/20 px-2.5 py-0.5 rounded-full mt-2.5 transition-all group-hover:scale-105"><?php echo e($cat->children->count()); ?> <?php echo e(__('global.sub_categories')); ?></span>
                        <?php endif; ?>
                    </div>
                </a>
            <?php endif; ?>
...
<div x-data="productFilter()" x-init="init()" class="sticky top-12 sm:top-14 lg:top-20 z-40 bg-white/80 dark:bg-[#030712]/70 nav-blur border-b border-slate-250/30 dark:border-slate-900/60 shadow-[0_4px_30px_rgba(0,0,0,0.02)] transition-all duration-300">
    <div class="container py-2.5 sm:py-3">
        <div class="hidden md:flex items-center gap-2 lg:gap-3 flex-wrap">
            <span class="text-xs font-bold text-slate-500 dark:text-slate-450"><?php echo e(__('global.filter_title')); ?></span>
            <select x-model="filters.category_id" @change="applyFilters()" class="border border-slate-200/50 dark:border-slate-800/80 bg-white/50 dark:bg-slate-950/60 text-slate-700 dark:text-slate-355 rounded-lg px-3 py-1.5 text-xs focus:ring-2 focus:ring-brand-primary dark:focus:ring-accent focus:outline-none transition-all">
                <option value=""><?php echo e(__('global.filter_all_categories')); ?></option>
                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if(is_object($cat) && isset($cat->id)): ?>
            ...
            <select x-model="filters.sort" @change="applyFilters()" class="border border-slate-200/50 dark:border-slate-800/80 bg-white/50 dark:bg-slate-950/60 text-slate-700 dark:text-slate-355 rounded-lg px-3 py-1.5 text-xs focus:ring-2 focus:ring-brand-primary dark:focus:ring-accent focus:outline-none transition-all">
                <option value=""><?php echo e(__('global.filter_sort_newest')); ?></option>
                <option value="price_asc"><?php echo e(__('global.filter_sort_price_low')); ?></option>
                <option value="price_desc"><?php echo e(__('global.filter_sort_price_high')); ?></option>
                <option value="name_az"><?php echo e(__('global.filter_sort_name_az')); ?></option>
            </select>
            <input type="number" x-model="filters.min_price" placeholder="<?php echo e(__('global.filter_min_price')); ?>" class="w-20 border border-slate-200/50 dark:border-slate-800/80 bg-white/50 dark:bg-slate-950/60 text-slate-700 dark:text-slate-355 rounded-lg px-3 py-1.5 text-xs focus:ring-2 focus:ring-brand-primary dark:focus:ring-accent focus:outline-none transition-all">
            <input type="number" x-model="filters.max_price" placeholder="<?php echo e(__('global.filter_max_price')); ?>" class="w-20 border border-slate-200/50 dark:border-slate-800/80 bg-white/50 dark:bg-slate-950/60 text-slate-700 dark:text-slate-355 rounded-lg px-3 py-1.5 text-xs focus:ring-2 focus:ring-brand-primary dark:focus:ring-accent focus:outline-none transition-all">
            <button @click="applyFilters()" :disabled="loading" class="bg-gradient-to-r from-brand-primary to-accent hover:from-brand-hover hover:to-accent-hover text-white px-3.5 py-1.5 rounded-lg text-xs font-extrabold transition disabled:opacity-50 flex items-center gap-1.5 shadow-md shadow-brand-primary/10 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-brand-primary">
                ...
                <span x-show="!loading"><?php echo e(__('global.filter_apply')); ?></span>
            </button>
            <button @click="resetFilters()" class="text-slate-400 hover:text-slate-655 dark:hover:text-slate-250 px-2 py-1.5 text-xs font-bold transition focus:outline-none focus:ring-2 focus:ring-brand-primary rounded-lg"><?php echo e(__('global.filter_reset')); ?></button>
        </div>
        <div class="md:hidden flex items-center justify-between">
            <button @click="mobileOpen = true" class="flex items-center gap-2 px-3 py-2 bg-brand-primary/10 text-brand-primary dark:text-accent rounded-lg text-xs font-bold hover:bg-brand-primary/20 transition-all border border-brand-primary/10">
                ...
                <span><?php echo e(__('global.filter_title')); ?></span>
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            </button>
            <span class="text-xs text-slate-500 font-bold" x-text="`${document.querySelectorAll('#product-grid .group').length} <?php echo e(__('global.product')); ?>`"></span>
        </div>
        <div x-show="mobileOpen" x-cloak class="fixed inset-0 z-50 md:hidden animate-fadeInUp">
            ...
            <div class="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-[#0b0f19]/95 backdrop-blur-2xl rounded-t-3xl shadow-2xl z-10 flex flex-col max-h-[85vh] border-t border-slate-200/20 dark:border-slate-800/60"
                ...
                <div class="p-4 border-b border-slate-200/60 dark:border-slate-900 flex items-center justify-between shrink-0">
                    <h3 class="font-extrabold text-sm text-slate-900 dark:text-white uppercase tracking-wider"><?php echo e(__('global.filter_title')); ?></h3>
                ...
                <div class="p-4 space-y-5 overflow-y-auto flex-1 text-start">
                    <div>
                        <label class="block text-xs font-extrabold mb-2 text-slate-500 dark:text-slate-450 uppercase tracking-wider"><?php echo e(__('global.filter_sort')); ?></label>
                        <div class="grid grid-cols-2 gap-2">
                            <label class="flex items-center justify-center p-2.5 border rounded-lg cursor-pointer transition text-xs font-bold"
                                ...
                                <?php echo e(__('global.filter_sort_newest')); ?>
                            ...
                            <label class="flex items-center justify-center p-2.5 border rounded-lg cursor-pointer transition text-xs font-bold"
                                ...
                                <?php echo e(__('global.filter_sort_price_low')); ?>
                            ...
                            <label class="flex items-center justify-center p-2.5 border rounded-lg cursor-pointer transition text-xs font-bold"
                                ...
                                <?php echo e(__('global.filter_sort_price_high')); ?>
                            ...
                            <label class="flex items-center justify-center p-2.5 border rounded-lg cursor-pointer transition text-xs font-bold"
                                ...
                                <?php echo e(__('global.filter_sort_name_az')); ?>

                            </label>
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-extrabold mb-2 text-slate-500 dark:text-slate-450 uppercase tracking-wider"><?php echo e(__('global.filter_category')); ?></label>
                    ...
                    <div>
                        <label class="block text-xs font-extrabold mb-2 text-slate-500 dark:text-slate-450 uppercase tracking-wider"><?php echo e(__('global.filter_min_price')); ?> - <?php echo e(__('global.filter_max_price')); ?></label>
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <button @click="resetFilters()" class="flex-1 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-350 px-4 py-3 rounded-xl text-sm font-extrabold transition hover:bg-slate-50 dark:hover:bg-slate-900/60 active:scale-95 focus:outline-none"><?php echo e(__('global.filter_reset')); ?></button>
                    </div>
                </div>
            </div>
        </div>
...
<section id="featured" class="anim-fade-up bg-slate-50/50 dark:bg-[#0b0f19]/30 py-8 sm:py-12 md:py-16">
    <div class="container">
        <div class="text-center mb-8 sm:mb-10">
            <h2 class="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-3"><?php echo e(__('global.featured_products')); ?></h2>
        ...
        <div id="product-grid">
            <?php echo $__env->make('shop.partials.product-grid', ['products' => $products, 'wishlistIds' => $wishlistIds ?? [], 'cartProductIds' => $cartProductIds ?? []], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        </div>
    </div>
</section>
    ...
    Alpine.data('productFilter', () => ({
        ...
        applyFilters() {
            ...
            fetch(requestUrl, {
                ...
                .catch(() => {
                    this.loading = false;
                    window.dispatchEvent(new CustomEvent('toast', { detail: { message: "<?php echo e(__('global.filter_error')); ?>", type: 'error' } }));
                });
        },
...
<?php echo $__env->make('layouts.store', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\GH\Desktop\Projects\elegant-store\resources\views/shop/home.blade.php ENDPATH**/ ?>